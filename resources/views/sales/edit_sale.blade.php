...
@section('breadcrumbs')
    {!! Breadcrumbs::render('edit_sale',$sale_id) !!}
...
@section('content')
    ...
    {{--Sale config--}}
    <?php $tax_amount = $settings['tax_rate'] ; ?>
    {{--Sale config--}}

    <div class ="alert alert-danger">
        Editing Sale <strong>EZPOS {{$sale_id }}</strong>
    </div>
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-btn bs-dropdown-to-select-group">
                        <button type="button" class="btn btn-default dropdown-toggle as-is bs-dropdown-to-select" data-toggle="dropdown">
                            <span data-bind="bs-drp-sel-label" id="bs-drp-sel-label">Sale</span>
                            {{--  <input type="hidden" name="selected_value" data-bind="bs-drp-sel-value" value="">--}}

                            <span class="caret"></span>
                            <span class="sr-only">Toggle Dropdown</span>
                        </button>
                        <ul class="dropdown-menu" id = "sale-type" data-selected-type="sale"  role="menu" style="">
        ...
        <div class ="col-sm-4  " >
                        ...
                        <div class="input-group contacts" style="padding-top:10px;padding-left:10px">
                            ...
                            <select id="customer" name="customer" class="add-customer-input keyboardLeft ui-autocomplete-input form-control" data-title="Customer Name" placeholder="Type customer name..." autocomplete="off">
                                ...
                                @foreach($customerList as $aCustomer)
                                    @if($aCustomer->id == $sales[0]->customer_id)
            ...
            <div class = "card" style="font-size:12px;">
                ...
                <div class="card">
                    ...
                    <strong>+Tax({{ $tax_amount }}%)</strong><span style="float: right"><strong data-tax="0" id="tax">$0.00</strong></span><br>
                    ...
                    <strong>Discount entire sale</strong><span style="float: right"><strong id=""><input id ="saleDiscountAmount" onkeyup="setAllItemToDiscount()" onkeydown="setAllItemToDiscount()" type ="number" placeholder="" style="max-width:45px;float: right" value ="0"></strong></span>
                </div>
...
@section('additionalJS')
    <script>
        var itemTotalInfo = [];
        var paymentAdded = 0;
        var taxRate = Number("{{ $tax_amount/100 }}");
        ...
        function prepopulateItem(item){
             ...
             if(document.getElementById("product-div-"+item.item_id) == null) {
                ...
                if(item.product_type!=2){
                    ...
                    var itemDescription = '<tr class="product-specific-description" data-index="'+item.item_id+'"  data-item-type="'+item.item_type+'"  id="product-div-' + item.item_id + '" data-rule-id="'+item.item_id+'">' +
                    ...
                    if(item.company_name!=null)
                        itemDescription +=  '<h6 class="media-heading"> by <a href="#">'+ item.company_name +'</a></h6>';
                    ...
                    itemDescription += '</div>' +
                            ...
                            ' <input type="number" min = "0" class="form-control quantity" id="product-' + item.item_id + '"  onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();" onchange = "addItemPriceToRegister(' + item.item_id + ')"  value="'+ Math.floor(item.quantity) +'">' +
                            '</td>' +
                    ...
                    $("#saleDiscountAmount").val(item.selling_price);
                    setSaleToDiscount();
                    addItemPriceToRegister(item.item_id);
                }
        ...
        function addItemToCart(item){
            ...
            if(itemTotalInfo[index].item_quantity<=0&&itemTotalInfo[index].product_type!=1){
                ...
                itemDescription += '</div>' +
                        '</div></td>' +
                        ' <td class="col-sm-1 col-md-1" style="text-align: center">' +
                        ' <input type="number" min = "0" class="form-control" id="product-' + item.getAttribute('data-id') + '"  onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();" onchange = "addItemPriceToRegister(' + item.getAttribute('data-id') + ')"  value="1">' +
                        '</td>' +
                        ...
                        '<span class="pe-7s-trash"></span> Remove<input type="hidden" id="cost-price-'+item.item_id+'" value="'+itemTotalInfo[index].cost_price+'" >' +
                        '</button></td></tr>' ;
        ...
        function setSaleToDiscount(){
            ...
            if(document.getElementById("product-div-discount") == null) {
                $('.no-items').remove();
                $('.add-payment').show();
                var itemDescription = '<tr style="background: aliceblue;" class="product-specific-description" data-index="0" id="product-div-discount" data-item-type="discount" data-rule-id="0">' +
                        '<td class="col-sm-8 col-md-6">' +
                        '<div class="media">' +
                        '' +
                        '<div class="media-body">' +
        ...
        function preLoadPayment(paymentInfo){
                ...
                if(paymentType!=null && tenderedAmount!=null)
                    $(".payment-history").append("<div class='card payment-log' data-id='"+paymentAdded+"' id='payment-"+paymentAdded+"' data-index-id="+paymentInfo.id+"  style='margin: 10px'><span class='pe-7s-close-circle' onclick='deletePayment("+paymentAdded+")' style='float:left'></span> <p id='payment-type-"+paymentAdded+"' style='float:left'>"+paymentType+"</p><p id='tendered-amount-"+paymentAdded+"' style='float:right' data-payment-amount='"+tenderedAmount+"'>$"+tenderedAmount+"</p><br></div>")
                paymentAdded++;
                calculateDue();
        ...
        function calculateDue(){
            ...
            due = Number(Number(total)-Number(totalPaidAmount)).toFixed(2);
