...
@section('pageTitle','Suspended Sale Detail Report')

@section('breadcrumbs')
    {!! Breadcrumbs::render('report_suspended_sale_detail') !!}
@stop
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-reports tablesorter stacktable small-only">
                    ...
                    <tr>
                        ...
                        <th align="right" class="header">Type</th>
                    </tr>
                    </thead>
                    <tbody id="data-table">
                        @foreach($sales as $aSale)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($aSale->sale_status==\App\Enumaration\SaleStatus::$LAYAWAY)
                                                   Lay Away
                                    @else if($aSale->sale_status==\App\Enumaration\SaleStatus::$ESTIMATE)
                                                   Estimate
                                    @endif
 ...
 <script>
     ...
     $(document).ready(function(e) {
         ...
         $('#order_by,  #start_date_formatted, #end_date_formatted').change(function() {
             ...
             $.ajax({
                 method: "POST",
                 url: "{{ route('report_suspended_detail_ajax') }}",
                 data: {
                 ...
                 data.sale.forEach(function(item){
                    ...
                    var payments = item.payment_logs;
                     ...
                     tableData+="</td>";
                     if(item.sale_status=="{{  \App\Enumaration\SaleStatus::$LAYAWAY }}")
                        tableData+="<td>LayAway</td>";
                     else if(item.sale_status=="{{  \App\Enumaration\SaleStatus::$ESTIMATE }}")
                        tableData+="<td>ESTIMATE</td>"
