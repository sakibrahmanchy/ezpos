...
@section('pageTitle','Discount Summary Report')

@section('breadcrumbs')
    {!! Breadcrumbs::render('report_discount_summary') !!}
@stop
                ...
                <table class="table table-bordered table-striped table-reports tablesorter stacktable small-only">
                    ...
                    <tbody id="data-table">
                        @foreach($sales as $aSale)
                            <tr>
                                <td>{{ $aSale->item_discount_percentage }}%</td>
                                <td>
                                    @if($aSale->subtotal>=0)
                                        ${{$aSale->subtotal}}
                                    @else
 ...
 <script>
     ...
     $(document).ready(function(e) {
         ...
         $('#order_by,  #start_date_formatted, #end_date_formatted').change(function() {
             ...
             $.ajax({
                 method: "POST",
                 url: "{{ route('report_discount_ajax') }}",
                 data: {
                 ...
                 data.sale.forEach(function(item){
                    tableData += "<tr>";
                    tableData+=" <td> "+ item.item_discount_percentage +"%</td>";
                    tableData+="<td>";
                    if(item.subtotal>=0)
                    ...
                    tableData+="</td>";
                    tableData+="<td>";
                    tableData+= parseInt(item.item_count);
                    tableData+="</td>";
                    tableData+="</tr>";
                 });
