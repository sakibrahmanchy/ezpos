...
@section('pageTitle','Employee Detail Report')

@section('breadcrumbs')
    {!! Breadcrumbs::render('report_employee_detail') !!}
@stop
                ...
                <div class="form-inline">
                    <div class="form-group" style="float:left">
                        <div class="input-group" >
                            <select class="form-control employeeSelect" id="employee_id">
                                @foreach($employees as $aEmployee)
                                    <option value="{{ $aEmployee->id }}">{{ $aEmployee->name }} </option>
                                @endforeach
                            </select>
                ...
                <table class="table table-bordered table-striped table-reports tablesorter stacktable small-only">
                    ...
                    @foreach($sales as $aSale)
                        ...
                        <tr>
                            ...
                            <a href="{{route('sale_edit',['sale_id'=>$aSale->id])}}"><span class="glyphicon glyphicon-edit"></span></a>
                                EZPOS {{$aSale->id}}</td>
                            ...
                            <td>
                                ...
                                @foreach($aSale->paymentLogs as $aPayment)
                                    {{$aPayment->payment_type}}: ${{$aPayment->paid_amount}}
                                @endforeach
...
@section('additionalJS')
    ...
    <script>
        ...
        $(document).ready(function(e) {
            ...
            $('#order_by,  #start_date_formatted, #end_date_formatted, #employee_id').change(function() {
                $('.data').addClass('hide');
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('report_employee_ajax') }}",
                    data: {
                    ...
                    data.sale.forEach(function(item){
                        ...
                        if(item.customer_id!=0){
                           if(item.customer!=undefined){
                               if(item.customer.first_name!=undefined)
                                   tableData+="<td> "+ item.customer.first_name+" ";
                               if(item.customer.last_name!=undefined)
                                   tableData+=item.customer.last_name +"";
                               tableData+="</td>";
                           }

                        }else{
                            tableData+=" <td> No Customer</td> ";
                        }
                        tableData+= "<td>";
