...
@section('pageTitle','Sale Hourly Summary Report')

@section('breadcrumbs')
    {!! Breadcrumbs::render('report_sale_summary_hourly') !!}
@stop
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-reports tablesorter stacktable small-only">
                    ...
                    @foreach($sales as $aSale)
                        <tr>
                            <td>{{ $aSale->starttime }} - {{ $aSale->endtime }} </td>
                            <td>
                                @if($aSale->subtotal>=0)
                                    ${{$aSale->subtotal}}
                                @else
    ...
    <script>
        ...
        $(document).ready(function(e) {
            ...
            $('#sale_type,  #start_date_formatted, #end_date_formatted').change(function() {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('report_sale_hourly_ajax') }}",
                    data: {
                    ...
                    data.sale.forEach(function(item){
                        tableData += "<tr>";
                        tableData+=" <td> "+ item.starttime+ " - " + item.endtime +"</td>";
                        tableData+="<td>";
                        if(item.subtotal>=0)
